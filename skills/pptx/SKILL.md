---
name: pptx
description: "演示文稿创建、编辑和分析。当 Claude 需要处理演示文稿（.pptx 文件）时，用于：(1) 创建新演示文稿，(2) 修改或编辑内容，(3) 使用布局，(4) 添加注释或演讲者备注，或任何其他演示文稿任务"
...
用户可能会要求您创建、编辑或分析 .pptx 文件的内容。.pptx 文件本质上是包含 XML 文件和其他您可以读取或编辑的资源的 ZIP 存档。您有不同的工具和工作流程可用于不同的任务。
...
### 原始 XML 访问
您需要原始 XML 访问来处理：注释、演讲者备注、幻灯片布局、动画、设计元素和复杂格式。对于这些功能中的任何一个，您需要解包演示文稿并读取其原始 XML 内容。
...
从头创建新 PowerPoint 演示文稿时，使用 **html2pptx** 工作流程将 HTML 幻灯片转换为具有准确定位的 PowerPoint。

### 设计原则

**关键**：在创建任何演示文稿之前，分析内容并选择适当的设计元素：
1. **考虑主题**：这个演示文稿是关于什么的？它暗示什么语气、行业或情绪？
2. **检查品牌**：如果用户提及公司/组织，考虑他们的品牌颜色和标识
...
- **构建您的调色板**：选择 3-5 种协调的颜色（主导颜色 + 支持色调 + 强调色）
- **确保对比**：文本必须在背景上清晰可读

**示例调色板**（使用这些来激发创造力 - 选择一个、调整它或创建您自己的）：

1. **Classic Blue（经典蓝）**：深海军蓝（#1C2833）、板岩灰（#2E4053）、银色（#AAB7B8）、米白（#F4F6F6）
2. **Teal & Coral（青色与珊瑚）**：青色（#5EA8A7）、深青色（#277884）、珊瑚（#FE4447）、白色（#FFFFFF）
3. **Bold Red（大胆红）**：红色（#C0392B）、亮红（#E74C3C）、橙色（#F39C12）、黄色（#F1C40F）、绿色（#2ECC71）
4. **Warm Blush（温暖腮红）**：紫灰色（#A49393）、腮红（#EED6D3）、玫瑰（#E8B4B8）、奶油色（#FAF7F2）
5. **Burgundy Luxury（勃艮第奢华）**：勃艮第（#5D1D2E）、深红（#951233）、铁锈色（#C15937）、金色（#997929）
6. **Deep Purple & Emerald（深紫与翡翠）**：紫色（#B165FB）、深蓝（#181B24）、翡翠（#40695B）、白色（#FFFFFF）
7. **Cream & Forest Green（奶油与森林绿）**：奶油色（#FFE1C7）、森林绿（#40695B）、白色（#FCFCFC）
8. **Pink & Purple（粉红与紫）**：粉红（#F8275B）、珊瑚（#FF574A）、玫瑰（#FF737D）、紫色（#3D2F68）
9. **Lime & Plum（青柠与李子）**：青柠（#C5DE82）、李子（#7C3A5F）、珊瑚（#FD8C6E）、蓝灰（#98ACB5）
10. **Black & Gold（黑金）**：金色（#BF9A4A）、黑色（#000000）、奶油色（#F4F6F6）
11. **Sage & Terracotta（鼠尾草与陶土）**：鼠尾草（#87A96B）、陶土（#E07A5F）、奶油色（#F4F1DE）、炭灰（#2C2C2C）
12. **Charcoal & Red（炭灰与红）**：炭灰（#292929）、红色（#E33737）、浅灰（#CCCBCB）
13. **Vibrant Orange（充满活力的橙色）**：橙色（#F96D00）、浅灰（#F2F2F2）、炭灰（#222831）
14. **Forest Green（森林绿）**：黑色（#191A19）、绿色（#4E9F3D）、深绿（#1E5128）、白色（#FFFFFF）
15. **Retro Rainbow（复古彩虹）**：紫色（#722880）、粉红（#D72D51）、橙色（#EB5C18）、琥珀（#F08800）、金色（#DEB600）
16. **Vintage Earthy（复古土色）**：芥末色（#E3B448）、鼠尾草（#CBD18F）、森林绿（#3A6B35）、奶油色（#F4F1DE）
17. **Coastal Rose（海岸玫瑰）**：旧玫瑰（#AD7670）、海狸色（#B49886）、蛋壳（#F3ECDC）、灰绿（#BFD5BE）
18. **Orange & Turquoise（橙色与青绿）**：浅橙（#FC993E）、灰青绿（#667C6F）、白色（#FCFCFC）
...
### 布局提示
**创建包含图表或表格的幻灯片时：**
- **两列布局（首选）**：使用跨越全宽的标题，然后下方两列 - 一列中的文本/项目符号和另一列中的特色内容。这提供了更好的平衡并使图表/表格更易读。使用具有不等列宽的 flexbox（例如，40%/60% 分割）以优化每种内容类型的空间。
- **全幻灯片布局**：让特色内容（图表/表格）占据整个幻灯片以获得最大影响和可读性
- **永远不要垂直堆叠**：不要将图表/表格放在单列文本下方 - 这会导致可读性差和布局问题

### 工作流程
1. **必需 - 读取整个文件**：从头到尾完整读取 [`html2pptx.md`](html2pptx.md)。**读取此文件时切勿设置任何范围限制。** 在继续演示文稿创建之前，读取完整的文件内容以获取详细语法、关键格式规则和最佳实践。
...
编辑现有 PowerPoint 演示文稿中的幻灯片时，您需要使用原始 Office Open XML（OOXML）格式。这涉及解包 .pptx 文件、编辑 XML 内容并重新打包它。

### 工作流程
1. **必需 - 读取整个文件**：从头到尾完整读取 [`ooxml.md`](ooxml.md)（~500 行）。**读取此文件时切勿设置任何范围限制。** 在进行任何演示文稿编辑之前，读取完整的文件内容以获取有关 OOXML 结构和编辑工作流程的详细指导。
...
1. **提取模板文本并创建视觉缩略图网格**：
   ...
   * 读取 `template-content.md`：读取整个文件以了解模板演示文稿的内容。**读取此文件时切勿设置任何范围限制。**
...
3. **基于模板清单创建演示文稿大纲**：
   ...
   * **关键：将布局结构与实际内容匹配**：
     ...
     - 引用布局：仅用于人们的实际引用（带归属），从不用于强调
     - 永远不要使用占位符多于内容的布局
     - 如果您有 2 个项目，不要强制它们进入 3 列布局
     - 如果您有 4+ 个项目，考虑拆分为多张幻灯片或使用列表格式
   * 在选择布局之前计算您的实际内容片段
   ...
   * 示例模板映射：
      ```
      # 要使用的模板幻灯片（从 0 开始索引）
      # 警告：验证索引在范围内！具有 73 张幻灯片的模板的索引为 0-72
...
5. **使用 `inventory.py` 脚本提取所有文本**：
   * **运行清单提取**：
     ...
     ```
   * **读取 text-inventory.json**：读取整个 text-inventory.json 文件以了解所有形状及其属性。**读取此文件时切勿设置任何范围限制。**
...
6. **生成替换文本并将数据保存到 JSON 文件**
   ...
   - **验证**：replace.py 脚本将验证替换 JSON 中的所有形状在清单中存在
   ...
   - **基本格式规则**：
     ...
     - 在与默认值不同时包含字体属性（例如，`"font_size": 14.0`、`"font_name": "Lora"`）
     - 颜色：使用 RGB 的 `"color": "FF0000"` 或主题颜色的 `"theme_color": "DARK_1"`
...
7. **使用 `replace.py` 脚本应用替换**
   ...
   错误：替换文本在这些形状中使溢出变得更糟：
     - slide-0/shape-2：溢出恶化了 1.25"（之前是 0.00"，现在是 1.25"）
   ```
...
**功能**：
- 创建：`thumbnails.jpg`（或大型幻灯片的 `thumbnails-1.jpg`、`thumbnails-2.jpg` 等）
...
要直观地分析 PowerPoint 幻灯片，请使用两步过程将它们转换为图像：
