...
description: 用于创建针对 Slack 优化的动画 GIF 的知识和实用工具。提供约束、验证工具和动画概念。当用户请求为 Slack 制作动画 GIF（如"为 Slack 制作一个 X 做 Y 的 GIF"）时使用。
...
要有创意和详细！一个好的 Slack GIF 应该看起来精致，而不像占位符图形。
...
- 开始位置：帧边界之外
- 结束位置：目标位置
- 使用 `interpolate()` 与 `easing='ease_out'` 实现平滑停止
...
仅当被要求缩小文件大小时，实施以下几种方法：

1. **更少的帧** - 较低的 FPS（10 而不是 20）或较短的持续时间
...
**关于用户上传的注意事项**：此技能不包含预构建的图形，但如果用户上传图像，请使用 PIL 加载并处理它 - 根据他们的请求解释他们是想直接使用它还是仅作为灵感。

要有创意！组合概念（弹跳 + 旋转、脉冲 + 滑动等）并充分利用 PIL 的功能。
