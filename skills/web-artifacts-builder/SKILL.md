...
description: 使用现代前端 Web 技术（React、Tailwind CSS、shadcn/ui）创建精心制作的多组件 claude.ai HTML 工件的工具套件。用于需要状态管理、路由或 shadcn/ui 组件的复杂工件 - 不用于简单的单文件 HTML/JSX 工件。
...
非常重要：为避免通常被称为"AI 糊弄"的东西，避免使用过多的居中布局、紫色渐变、统一的圆角和 Inter 字体。
...
要构建工件，编辑生成的文件。有关指导，请参见下面的**常见开发任务**。
...
要测试/可视化工件，请使用可用工具（包括其他技能或内置工具，如 Playwright 或 Puppeteer）。通常，避免预先测试工件，因为它会在请求和完成工件可见之间增加延迟。如果被请求或出现问题，请在呈现工件后进行测试。
