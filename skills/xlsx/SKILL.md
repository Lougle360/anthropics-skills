---
name: xlsx
description: "具有公式、格式化、数据分析和可视化支持的综合电子表格创建、编辑和分析。当 Claude 需要处理电子表格（.xlsx、.xlsm、.csv、.tsv 等）时，用于：(1) 创建带有公式和格式的新电子表格，(2) 读取或分析数据，(3) 在保留公式的同时修改现有电子表格，(4) 电子表格中的数据分析和可视化，或 (5) 重新计算公式"
...
#### 硬编码的文档要求
- 在旁边的单元格中注释或（如果表的末尾）。格式："来源：[系统/文档]，[日期]，[具体参考]，[URL（如适用）]"
- 示例：
  - "来源：公司 10-K，FY2024，第 45 页，收入说明，[SEC EDGAR URL]"
  - "来源：公司 10-Q，2025 年第二季度，附件 99.1，[SEC EDGAR URL]"
  - "来源：Bloomberg Terminal，2025 年 8 月 15 日，AAPL US Equity"
...
**公式重新计算需要 LibreOffice**：您可以假设已安装 LibreOffice，以便使用 `recalc.py` 脚本重新计算公式值。脚本在首次运行时自动配置 LibreOffice
...
3. **修改**：添加/编辑数据、公式和格式
4. **保存**：写入文件
5. **重新计算公式（如果使用公式则为必需）**：使用 recalc.py 脚本
   ```bash
